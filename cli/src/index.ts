...
program
  ...
  .hook('preAction', async (thisCommand, actionCommand) => {
    ...
    const forceApiFromEnv =
      process.env.LANONASIS_FORCE_API === 'true' ||
      process.env.CLI_FORCE_API === 'true' ||
      process.env.ONASIS_FORCE_API === 'true';
    const forceApiFromConfig =
      cliConfig.get<boolean>('forceApi') === true ||
      cliConfig.get<string>('connectionTransport') === 'api';
  ...
  .action(async () => {
    ...
    if (isAuth) {
      ...
      try {
        ...
        if (cached?.email) console.log(`Email: ${cached.email} (cached)`);
      }
    } else {
    ...
    if (cliConfig.hasManualEndpointOverrides()) {
      ...
      const services = cliConfig.get<Record<string, string>>('discoveredServices');
      if (services) {
...
program
  ...
  .action(async () => {
    await cliConfig.init();
    const verification = await cliConfig.verifyCurrentCredentialsWithServer().catch((error: unknown) => ({
      valid: false,
    ...
    if (isAuth) {
      try {
        ...
        const cached = await cliConfig.getCurrentUser();
        if (cached?.email) console.log(`User: ${cached.email} (cached)`);
      }
      return;
    }
