...
export function mcpCommands(program: Command) {
  ...
  mcp.command('status')
    ...
    .action(async () => {
      ...
      let resolvedHealthUrl: string | undefined;

      try {
        const axios = (await import('axios')).default;

        const normalizeMcpHealthUrl = (inputUrl: string): string => {
        ...
        let healthProbeBase: string;
        if (status.mode === 'websocket') {
          healthProbeBase = config.get<string>('mcpWebSocketUrl') ?? config.getMCPServerUrl();
        } else if (status.mode === 'remote') {
          healthProbeBase = config.get<string>('mcpServerUrl') ?? config.getMCPRestUrl();
        } else {
