...
// Import the CLIConfigData type from config.ts
type CLIConfigData = {
  discoveredServices?: {
    auth_base: string;
    memory_base: string;
...
export function configCommands(program: Command): void {
  ...
  program
    ...
    .action(async () => {
      ...
      const lastDiscovery = config.get<string>('lastServiceDiscovery');
      const lastManualUpdate = config.get<string>('lastManualEndpointUpdate');
      ...
      try {
        ...
        if (services) {
          ...
          if (config.hasManualEndpointOverrides()) {
            ...
            const lastUpdate = config.get<string>('lastManualEndpointUpdate');
