...
interface GetMemoriesParams extends ApiGetMemoriesParams {
  page: number;
  limit: number;
  sort?: ApiGetMemoriesParams['sort'];
  order?: ApiGetMemoriesParams['order'];
  tags?: string;
}
...
type QueryScopeOption = 'personal' | 'team' | 'organization' | 'hybrid';

interface IntelligenceContextOptions extends JsonOutputOption {
...
interface IntelligenceFindRelatedOptions extends IntelligenceContextOptions {
  limit?: string;
  threshold?: string;
}

interface IntelligenceAnalyzeOptions extends IntelligenceContextOptions {
  days?: string;
  /**
   * Card 2 (2026-07-18): commander turns `--no-mcp` into `mcp: false`.
   * Local override for parity flag (global lives on root program).
   */
  mcp?: boolean;
}

interface IntelligenceExtractOptions extends IntelligenceContextOptions {
  topic?: string;
  type?: MemoryType;
  maxMemories?: string;
  /**
   * Card 2 (2026-07-18): local override for `--no-mcp` parity flag.
   */
  mcp?: boolean;
}

interface IntelligenceDetectOptions extends IntelligenceContextOptions {
  threshold?: string;
  maxPairs?: string;
  /**
   * Card 2 (2026-07-18): destructive dry-run / execute / confirm flags
   * that drive `mode: 'delete'` in the EF body.
   */
  dryRun?: boolean;
  delete?: boolean;
  confirm?: boolean;
  duplicateIds?: string;
  /** Local override for `--no-mcp` parity flag. */
  mcp?: boolean;
}
...
// Card 2 (2026-07-18): extend transport with a "rest" mode that hits the
// onasis-core REST Edge Function path (POST /api/v1/intelligence/*) so the
// `--no-mcp` CLI flag has a real path to fall back to, and the same query
// can be replayed through both MCP and REST for parity testing.
type TransportResolver = (noMcp: boolean) => Promise<IntelligenceTransport>;
let transportResolver: TransportResolver = async (noMcp) => {
 ...
 */
export const __setIntelligenceTransportResolver = (next: TransportResolver): void => {
 ...
 */
const resolveIntelligenceContextTransport = async (
  program: Command,
  localNoMcp: boolean | undefined,
): Promise<IntelligenceTransport> => {
...
const QUERY_SCOPE_CHOICES: ReadonlyArray<QueryScopeOption> = [
  'personal',
  'team',
  'organization',
  'hybrid',
...
const coerceMemoryType = (value: unknown): MemoryType | undefined => {
  if (typeof value !== 'string') return undefined;
  const normalized = value.trim().toLowerCase();

  // Backward compatibility for older docs/examples.
  if (normalized === 'conversation') return 'context';

  if ((MEMORY_TYPE_CHOICES as ReadonlyArray<string>).includes(normalized)) {
...
const coerceQueryScope = (value: unknown): QueryScopeOption | undefined => {
  if (typeof value !== 'string') return undefined;
  const normalized = value.trim().toLowerCase();

  if ((QUERY_SCOPE_CHOICES as ReadonlyArray<string>).includes(normalized)) {
    return normalized as QueryScopeOption;
  }

  return undefined;
};

const parseMemoryTypesOption = (value?: string): MemoryType[] | undefined => {
  if (!value?.trim()) return undefined;

  const parsed = value
    .split(',')
    .map((entry) => coerceMemoryType(entry))
    .filter((entry): entry is MemoryType => Boolean(entry));
...
const buildIntelligenceContextPayload = (
  ...
  if (options.scope) {
    ...
    if (!scope) {
      throw new Error(
        `Invalid scope \"${options.scope}\". Expected one of: ${QUERY_SCOPE_CHOICES.join(', ')}`
      );
    }
...
export function memoryCommands(program: Command): void {
    ...
    .action(async (options: CreateMemoryOptions) => {
      try {
        ...
        if (json) {
          ...
          if (!content && typeof parsed.content === 'string') content = parsed.content;
          ...
          if (!type && parsedType !== undefined) {
            ...
            if (!coerced) {
              throw new Error(
                `Invalid memory type in --json payload. Expected one of: ${MEMORY_TYPE_CHOICES.join(', ')}`
              );
            }
            type = coerced;
          }
          ...
          if (!tags) {
            if (Array.isArray(parsed.tags)) {
              tags = parsed.tags.map((t: unknown) => String(t)).join(',');
          ...
          if (!topicId && typeof parsedTopic === 'string') topicId = parsedTopic;
        }
        ...
        const resolvedType: MemoryType = type ?? 'context';
  ...
  program
    ...
    .action(async (options: { title?: string; type?: string; tags?: string; testSummary?: string }) => {
      try {
        const spinner = ora('Collecting session info...').start();

        const cwd = process.cwd();

        const runGit = async (cmd: string): Promise<string | null> => {
          try {
    ...
    .action(async (options: ListMemoryOptions) => {
      try {
        ...
        const params: GetMemoriesParams = {
          ...
          sort: (options.sort as GetMemoriesParams['sort']) || 'created_at',
          order: (options.order as GetMemoriesParams['order']) || 'desc'
        };
      ...
      try {
        ...
        for (const threshold of thresholdPlan) {
          try {
            ...
            const rawAttemptResults = (attempt.results || attempt.data || []) as unknown[];
  ...
  intelligence
    ...
    .option(
      '--memory-types <types>',
      `Optional comma-separated memory types (${MEMORY_TYPE_CHOICES.join(', ')})`
    )
    .option('--json', 'Output raw JSON payload')
    .action(async (options: IntelligenceContextOptions) => {
      try {
  ...
  intelligence
    ...
    .option('--scope <scope>', `Optional query scope (${QUERY_SCOPE_CHOICES.join(', ')})`)
    .option('--json', 'Output raw JSON payload')
    .action(async (memoryId: string, options: IntelligenceContextOptions & { max?: string }) => {
      try {
  ...
  intelligence
    ...
    .option(
      '--memory-types <types>',
      `Optional comma-separated candidate memory types (${MEMORY_TYPE_CHOICES.join(', ')})`
    )
    .option('--json', 'Output raw JSON payload')
    ...
    .action(async (options: IntelligenceDetectOptions) => {
      try {
        ...
        if (isDeleteMode) {
          ...
          const transport = await resolveIntelligenceContextTransport(
            intelligence,
            options.mcp === false,
          );
          const result = await postIntelligenceEndpoint<Record<string, unknown>>(
          ...
          if (dryRun) {
            console.log(
              chalk.yellow(
                `⚠ Dry-run only. ${dryRun && !options.confirm ? 'Pass --confirm to actually delete.' : ''}`,
              ),
            );
          }
          printIntelligenceResult(
            dryRun ? '🧪 Delete Dry-Run' : '🧨 Delete Executed',
            result,
            options,
          );
          return;
