...
const accessLevelChoices = ['public', 'authenticated', 'team', 'admin', 'enterprise'];
const serviceTypeChoices = ['all', 'specific'];
...
const parseOptionalPositiveInteger = (value, flagName) => {
    if (!value)
        return undefined;
...
const parseOptionalJsonObject = (value, flagName) => {
    if (!value)
        return undefined;
    let parsed;
    try {
...
const formatPermissions = (permissions) => permissions && permissions.length > 0 ? permissions.join(', ') : 'legacy:full_access';
const renderKeySummary = (apiKey) => {
...
const fetchConfiguredServices = async () => {
    ...
    const services = unwrapEnvelope(response);
    return Array.isArray(services) ? services : [];
};
const resolveServicePayload = async (options) => {
    let payload = {
        name: options.name,
        ...
        rate_limits: parseOptionalJsonObject(options.rateLimits, '--rate-limits')
    };
    ...
    const answers = await inquirer.prompt([
        ...
        },
        {
            ...
            filter: (input) => input.trim() ? Number.parseInt(input, 10) : undefined
        },
        {
            type: 'input',
            name: 'scopes',
            ...
            filter: (input) => parseCommaList(input)
        },
        {
            type: 'select',
            name: 'service_type',
        ...
        },
        {
            ...
            when: (answers) => (answers.service_type || payload.service_type) === 'specific' && configuredServices.length > 0 && payload.service_keys.length === 0,
            ...
            validate: (input) => input.length > 0 || 'Select at least one configured service'
        }
    ]);
    payload = {
        ...payload,
...
apiKeysCommand
    ...
    try {
        ...
        const apiKey = unwrapEnvelope(response);
        if (options.json) {
...
apiKeysCommand
    ...
    try {
        ...
        const apiKeys = unwrapEnvelope(response);
        if (options.json) {
            ...
            const answers = await inquirer.prompt([
                {
                    ...
                    validate: (input) => input.trim().length > 0 || 'At least one scope is required',
                    filter: (input) => parseCommaList(input)
                }
            ]);
...
servicesCommand
    ...
    try {
        ...
        const rateLimits = parseOptionalJsonObject(options.rateLimits, '--rate-limits');
            ...
            const answers = await inquirer.prompt([
                {
                    ...
                    validate: (input) => input.length > 0 || 'Select at least one service'
                }
            ]);
