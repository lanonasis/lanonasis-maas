...
export class MemoryService {
     ...
     */
    async searchMemories(query, organizationId, filters = {}) {
        const startTime = Date.now();
        try {
     ...
     */
    async getMemoryCount(organizationId) {
     ...
     */
    async getMemoryStats(organizationId) {
        const startTime = Date.now();
        try {
            ...
            const totalSizeBytes = sizeStats?.reduce((total, item) => total + new Blob([item.content]).size, 0) || 0;
     ...
     */
    async bulkDeleteMemories(memoryIds, organizationId) {
        const startTime = Date.now();
        const failedIds = [];
        let deletedCount = 0;
        try {
     ...
     * Log analytics event
     */
    async logAnalytics(organizationId, userId, action, resourceType, resourceId, metadata = {}) {
        try {
            await this.supabase
