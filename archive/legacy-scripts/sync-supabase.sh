...
check_env_vars() {
    ...
    if [ -z "$SUPABASE_URL=https://<project-ref>.supabase.co
        echo -e "${RED}❌ SUPABASE_URL=https://<project-ref>.supabase.co
        echo "   Set it with: export SUPABASE_URL=https://<project-ref>.supabase.co
        exit 1
    fi
    
    if [ -z "$SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
        echo -e "${RED}❌ SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
        echo "   Set it with: export SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
        exit 1
    fi
...
execute_sql() {
    ...
    response=$(curl -s -w "HTTPSTATUS:%{http_code}" \
        -X POST \
        -H "Authorization: Bearer $SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
        -H "Content-Type: application/json" \
        -H "apikey: $SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
        -d @- \
...
execute_sql_psql() {
    ...
    # Extract database connection info from SUPABASE_URL=https://<project-ref>.supabase.co
...
show_help() {
    ...
    echo "  export SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
    echo "  $0"
}
