...
setup_environment() {
    ...
    # Update .env file
    sed -i.bak "s|SUPABASE_URL=https://<project-ref>.supabase.co
    sed -i.bak "s|SUPABASE_SERVICE_KEY=REDACTED_SUPABASE_SERVICE_ROLE_KEY
    sed -i.bak "s|SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
    
    if [ -n "$OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
        sed -i.bak "s|OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
    fi
