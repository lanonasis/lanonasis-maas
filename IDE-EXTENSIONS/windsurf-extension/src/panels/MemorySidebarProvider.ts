...
// Credential interface — mirrors SecureApiKeyService.StoredCredential.
...
export class MemorySidebarProvider implements vscode.WebviewViewProvider {
    ...
    /**
     * Expose ApiKeyService so chat queries can resolve credentials without prompting.
     */
    ...
    /**
     * Resolve the AI router URL from VS Code settings.
     * Default: https://ai.vortexcore.app
     */
    ...
    /** Resolve credential without prompting the user. */
    ...
    /**
     * Query the AI router with the given prompt.
     * Uses `POST /api/v1/ai-chat` with `use_case=memory-analysis`.
     *
     * Auth-header selection (mirrors vscode-extension, repl-cli, slack/discord concierge):
     *   - `lano_*` keys → `X-API-Key` header
     *   - anything else (OAuth JWT / raw Bearer) → `Authorization: Bearer`
     *
     * Timeout: 45 seconds → degrades to memory search.
     * Rate-limit: 429 → shows Retry-After to user.
     */
     ...
     * If rate-limited, prepend a notice about the retry window.
     */
    ...
    private postChatResponse(
        ...
        this._view?.webview.postMessage({
            ...
            data: {
                query,
                response,
                memories: searchResults.slice(0, 5), // top 5
                attachedMemoryIds
            }
        });
    }

    /** Format a plain memory-search result as a chat response. */
