...
const MemoryCache_1 = require("./services/MemoryCache");
...
const MCPDiscoveryService_1 = require("./services/MCPDiscoveryService");
const MemoryCacheBridge_1 = require("./bridges/MemoryCacheBridge");
async function activate(context) {
    ...
    if (enableMCP && mcpAutoDiscover) {
        try {
            mcpDiscoveryService = await (0, MCPDiscoveryService_1.createMCPDiscoveryService)(outputChannel);
    ...
    const memoryCache = new MemoryCache_1.MemoryCache(context, outputChannel);
    const memoryCacheBridge = new MemoryCacheBridge_1.MemoryCacheBridge(memoryCache, memoryService, outputChannel);
    ...
    if (useEnhancedUI) {
        sidebarProvider = new EnhancedSidebarProvider_1.EnhancedSidebarProvider(context.extensionUri, memoryService, apiKeyService, memoryCacheBridge);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('lanonasis.showMCPStatus', async () => {
            if (mcpDiscoveryService) {
                ...
                const action = await vscode.window.showInformationMessage('MCP auto-discovery is disabled or no server found.', 'Run Discovery', 'Configure');
                if (action === 'Run Discovery') {
                    const newService = await (0, MCPDiscoveryService_1.createMCPDiscoveryService)(outputChannel);
