/**
 * Lightweight, self-contained secrets redactor used by the local memory
 * write pipeline. Pattern source: packages/openclaw-plugin/extraction/
 * secret-redactor.ts (MIT, internal). We re-implement here rather than
 * reaching across the monorepo so the local-memory module stays
 * installable in isolation.
 *
 * Coverage: API keys (Anthropic, OpenAI, Stripe, AWS, Google, Notion,
 * LanOnasis, ElevenLabs), tokens (GitHub, Slack/Notion-like, JWT, Bearer),
 * SSH private keys, database URLs, hex secrets, and explicit
 * `KEY=value` assignments. Patterns match what openclaw-plugin ports
 * from pk-pi-hermes-evolve engine.ts scanForSecrets().
 */
